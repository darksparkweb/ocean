...
export default class Sketch {
  ...
  composerPass(){
    ...
    this.myEffect = {
      ...
      void main(){
        vec2 newUV = vUv;
        float area = smoothstep(0.1,0.,vUv.y)*2. - 1.;
        float area2 = smoothstep(1.,0.85,vUv.y)*2. - 0.4;
        // area = pow(area, 4.);
        float noise = 0.5*(cnoise(vec3(vUv*10., time/5.)) + 1.);
        float n = smoothstep(0.5,0.5, noise + area2);
  ...
  render() {
    ...
    this.customPass.uniforms.scrollPos.value = this.currentScroll;
